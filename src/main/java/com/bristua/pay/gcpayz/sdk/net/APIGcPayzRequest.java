...
import com.bristua.pay.gcpayz.sdk.util.GcpayzKit;
...
public class APIGcPayzRequest {
    ...
    public static URL buildURLWithSign(String url, Map<String, Object> params, RequestOptions options) throws APIConnectionException {
        ...
        params.put(GcPayzSDK.API_VERSION_NAME, options.getVersion());
        params.put(GcPayzSDK.API_SIGN_TYPE_NAME, options.getSignType());
        String requestTime = currentTimeString();
        params.put(GcPayzSDK.API_REQ_TIME_NAME, requestTime);
        String signature;
        try {
            signature = buildGcPayzSignature(params, options);
        } catch (IOException e) {
            throw new APIConnectionException("生成GcPayz请求签名异常", e);
        }

        if (signature != null) {
            params.put(GcPayzSDK.API_SIGN_NAME, signature);
        }
        ...
        try {
            ...
            throw new APIConnectionException("生成 GcPayzSDK 请求URL异常", e);
        }
    }

    private static HttpContent buildContent (
            APIResource.RequestMethod method, Map<String, Object> params, RequestOptions options) throws GcPayzException {
    ...
    protected static String buildGcPayzSignature(Map<String, Object> params, RequestOptions options)
            throws IOException {

        String signType = options.getSignType();
        if("MD5".equalsIgnoreCase(signType)) {
            return GcpayzKit.getSign(params, options.getApiKey());
